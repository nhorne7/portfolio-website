...
canvas.addEventListener("mousedown", e=>{ const{x,y}=getCanvasPos(e); for(const p of pendulums){const b2=p.getBob2(); if(Math.hypot(x-b2.x,y-b2.y)<DRAG_RADIUS){draggingPendulum=p;p.isDragging=true;p.trace=[];p.dragTo(x,y);break;}} });
...
canvas.addEventListener("mouseup", ()=>{ if(draggingPendulum){draggingPendulum.isDragging=false;draggingPendulum=null;} });
canvas.addEventListener("mouseleave", ()=>{ if(draggingPendulum){draggingPendulum.isDragging=false;draggingPendulum=null;} pendulums.forEach(p=>p.isHovered=false); });
canvas.addEventListener("touchstart",e=>{e.preventDefault();const{x,y}=getCanvasPos(e);for(const p of pendulums){const b2=p.getBob2();if(Math.hypot(x-b2.x,y-b2.y)<DRAG_RADIUS){draggingPendulum=p;p.isDragging=true;p.trace=[];p.dragTo(x,y);break;}}},{passive:false});
...
canvas.addEventListener("touchend",e=>{e.preventDefault();if(draggingPendulum){draggingPendulum.isDragging=false;draggingPendulum=null;}},{passive:false});
canvas.addEventListener("touchcancel",e=>{e.preventDefault();if(draggingPendulum){draggingPendulum.isDragging=false;draggingPendulum=null;}},{passive:false});
