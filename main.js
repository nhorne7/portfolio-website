...
class DoublePendulum {
  ...
  dragTo(x, y) {
    ...
    if (this._prevDragX !== null) {
      // Exponential smoothing to reduce jitter — higher alpha = more responsive
      const alpha = 0.6;
  ...
  releaseWithMomentum() {
    ...
    if (Math.hypot(vx, vy) > 0.5) {
      ...
      const MAX_OMEGA = 25;
      this.omega1 = Math.max(-MAX_OMEGA, Math.min(MAX_OMEGA, (vx * t1x + vy * t1y) / L1px / dt));
      this.omega2 = Math.max(-MAX_OMEGA, Math.min(MAX_OMEGA, (vx * t2x + vy * t2y) / L2px / dt));
    }
